...
turtles-own [mkt culture growth-rate coop-with-same? coop-with-diff?]  ;; Market share, culture, firm growth rate, probability of coop/defect
globals [total-mkt max-mkt mean-mkt] ;; Total market share, max-mkt. share, mean-mkt. share
...
to setup-firms
  create-ethnos initial-number-ethnos [
   ...
   set growth-rate median (list 0(random-normal 10 5) 15 ) ;use normal distribution in TA note to radomize number within min-max
   set coop-with-same? (random-float 1.0 < chance-coop-with-same)  ;;;; NEED TO MAKE COOP/DEFECT RATES DIFFERENT FOR EACH TYPE
   set coop-with-diff? ((random-float .25) + 0.5 < chance-coop-with-diff)
  ]
  crt initial-number-egos [
   ...
   set growth-rate (random-normal 10 5)
  ...
  crt initial-number-cosmos [
   ...
   set coop-with-diff? (random-float 1.0 < chance-coop-with-diff)
  ]
  crt initial-number-altruists [
   ...
   ;set mkt (total-mkt/ initial-number-firms)
   set growth-rate (random-normal 10 5)
   set coop-with-same? (random-float 1.0 < chance-coop-with-same)
   set coop-with-diff? (random-float 1.0 < chance-coop-with-diff)
  ]
end
...
to seek-merger
  let target one-of turtles-on neighbors
  ask target [                ;; HOW TO MAKE THE RANGE THE ENTIRE FIELD?
    if breed = [breed] of myself [
      ;if coop-with-same? [
      ;  merge
      ;]
       ifelse coop-with-same? [
        merge
      ][ask myself [set cost ]]
      ...
      ]             ;; NEED TO DETERMINE HOW TO MAKE DEFECT COSTLY
      ]
  ]
end
...
to merge
  ...
  ;;spring layout
  ;;hatch new firm
  ifelse market-share > [market-share] of myself [
    let new-breed breed
  ][let new-breed [breed] of myself]
  let count-merge max list(merger-time [merger-time] of myself)
  hatch 1[
    set breed new-breed
    set merger-time count-merge  + 1
  ]
  ;;die
...
to mutate
  ...
  ;;   BE BREED OF LARGER FIRM ->> SAME COOP/DEFECT RATES
  ;;   HAVE MKT = (OLD MKT * 1.1)
  ;;
end
...
;; # OF MERGERS, TRACKING OF WHERE IN THE LIFECYCLE THE MKT. IS
@#$#@#$#@
GRAPHICS-WINDOW
266
13
