...
to check-status
  ask turtles   ;; firms compare internal/industry growth rates & merge if below, gain mkt. share if above
     [ ifelse growth-rate < mkt-growth
        [seek-merger]
        [set mkt mkt * (1 + (growth-rate - mkt-growth))^ ticks] ;; WHAT DO WE WANT THIS FUNCTION TO BE?
  ]
end

to seek-merger
  let target one-of turtles-on neighbors   ;; WILL THIS BE ENTIRE ENVIRONMENT OR ONLY 8 PATCHES EACH DIRECTION?
  ask target [
    if breed = [breed] of myself [
      ifelse coop-with-same?
        [merge]
        [ask myself [set cost?]  ;; WHAT DO WE WANT COST TO BE?

      if breed != [breed] of myself [
        ifelse coop-with-diff?
          [merge]
          [ask myself [set cost?]]
      ]
      ]
