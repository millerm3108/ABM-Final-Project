...
to setup-firms
  crt initial-number-ethnos [
   ...
   set coop-with-diff? (random-float .25 < chance-coop-with-diff)
  ]
...
to seek-merger
  ask turtles-on neighbors [                ;; HOW TO MAKE THE RANGE THE ENTIRE FIELD?
    if breed = [breed] of myself [
      if [coop-with-same?] of myself [
        ask myself [merge]
      ]
      if breed != [breed] of myself [
        if [coop-with-diff?] of myself [
         ask myself [merge] ;
        ]
